...
> Theses steps will run an Edge Server in your local machine and connect to a MySQL data source. You can use the same steps to connect other supported data sources by using the specific dfcli parameters.
...
2. Verify Docker has been installed by running the following command and confirm the **Hello from Docker!** reply:
   ...
   ```
3. Open the Terminal and run the following commands to pull then run the latest Datafi CLI container image:
...
4. The previous step runs the cli container and installs `dfcli`. Use `dfcli` to generate the environment variable used while running the Edge Server. Run the following command in the bash shell from the previous step.
   ...
   > The above command adds a MySQL dataset. The parameters will be different for each data source. Run `dfcli dataset` to see the parameters for the available data sources.

   This step will connect to the data source with the provided credentials and inspect the schema. It will generate a `key.txt` file in the `es/out` folder you should backup off line. The `key.txt` file will be used in the next step when running the container. Use the link to add the dataset to your Datafi Workspace.

   After backing up the `key.txt` file and copying the link, enter `exit` to quite the CLI.
...
6. Use the link from the previous step to connect the Edge Server and add the dataset to your Datafi workspace. Note: Log into your workspace before clicking on the link. [signed up for a free Datafi account here](https://home.datafi.us/register).
